"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateCurrentUserDTO = void 0;
const openapi = require("@nestjs/swagger");
const class_transformer_1 = require("class-transformer");
const class_validator_1 = require("class-validator");
class UpdateCurrentUserDTO {
    static _OPENAPI_METADATA_FACTORY() {
        return { firstName: { required: false, type: () => String, minLength: 2 }, lastName: { required: false, type: () => String }, photo: { required: false, type: () => String }, phoneNumber: { required: false, type: () => String } };
    }
}
exports.UpdateCurrentUserDTO = UpdateCurrentUserDTO;
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsNotEmpty)({ message: 'First name cannot be null' }),
    (0, class_validator_1.MinLength)(2, { message: 'First name at least 2 characters' }),
    (0, class_validator_1.IsString)({ message: 'First name must be a string' }),
    __metadata("design:type", String)
], UpdateCurrentUserDTO.prototype, "firstName", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsString)({ message: 'Last name must be a string' }),
    __metadata("design:type", String)
], UpdateCurrentUserDTO.prototype, "lastName", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsString)({ message: 'Photo must be a string' }),
    __metadata("design:type", String)
], UpdateCurrentUserDTO.prototype, "photo", void 0);
__decorate([
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsPhoneNumber)('VN', { message: 'Invalid Phone number' }),
    (0, class_validator_1.IsString)({ message: 'Photo must be a string' }),
    __metadata("design:type", String)
], UpdateCurrentUserDTO.prototype, "phoneNumber", void 0);
//# sourceMappingURL=update-current-user.dto.js.map