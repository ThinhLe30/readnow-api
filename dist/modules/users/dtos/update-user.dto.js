"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateUserDTO = void 0;
const openapi = require("@nestjs/swagger");
const swagger_1 = require("@nestjs/swagger");
const class_transformer_1 = require("class-transformer");
const class_validator_1 = require("class-validator");
const common_enum_1 = require("../../../common/enum/common.enum");
let UpdateUserDTO = exports.UpdateUserDTO = class UpdateUserDTO {
    static _OPENAPI_METADATA_FACTORY() {
        return { email: { required: false, type: () => String }, password: { required: false, type: () => String, minLength: 2 }, firstName: { required: false, type: () => String, minLength: 2 }, lastName: { required: false, type: () => String }, photo: { required: false, type: () => String }, phoneNumber: { required: false, type: () => String }, role: { required: false, enum: require("../../../common/enum/common.enum").Role } };
    }
};
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsEmail)({}, { message: 'Invalid email' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "email", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.MinLength)(2, { message: 'Password at least 2 characters' }),
    (0, class_validator_1.IsString)({ message: 'password must be a string' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "password", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsNotEmpty)({ message: 'First name cannot be null' }),
    (0, class_validator_1.MinLength)(2, { message: 'First name at least 2 characters' }),
    (0, class_validator_1.IsString)({ message: 'First name must be a string' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "firstName", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsString)({ message: 'Last name must be a string' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "lastName", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsString)({ message: 'Photo must be a string' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "photo", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsPhoneNumber)('VN', { message: 'Invalid Phone number' }),
    (0, class_validator_1.IsString)({ message: 'Photo must be a string' }),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "phoneNumber", void 0);
__decorate([
    (0, swagger_1.ApiProperty)(),
    (0, class_transformer_1.Expose)(),
    (0, class_validator_1.IsOptional)(),
    (0, class_validator_1.ValidateIf)((obj, value) => value),
    (0, class_validator_1.IsEnum)(common_enum_1.Role),
    __metadata("design:type", String)
], UpdateUserDTO.prototype, "role", void 0);
exports.UpdateUserDTO = UpdateUserDTO = __decorate([
    (0, class_transformer_1.Exclude)()
], UpdateUserDTO);
//# sourceMappingURL=update-user.dto.js.map